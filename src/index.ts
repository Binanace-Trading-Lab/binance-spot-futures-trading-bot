...
async function main() {
  ...
  log("info", "boot", { project: "binance-spot-futures-trading-bot", venue: "binance", mode, kind: "dual" });

  if (mode === "live") {
    assertLiveAllowed(settings, args.confirmLive, "BINANCE");
    const broker = createCcxtBroker(settings, "BINANCE", "binance");
