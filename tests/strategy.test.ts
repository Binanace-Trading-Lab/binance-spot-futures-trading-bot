...
test("binance-spot-futures-trading-bot settings load", () => {
  const s = loadSettings(process.cwd());
  assert.equal(s.venue, "binance");
  assert.ok(s.risk.maxDailyLossUsd > 0);
});

test("binance-spot-futures-trading-bot paper strategy steps", async () => {
  const s = loadSettings(process.cwd());
...
test("binance-spot-futures-trading-bot risk guardian trips daily loss", () => {
  const s = loadSettings(process.cwd());
