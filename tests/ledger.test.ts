...
test("binance-spot-futures-trading-bot ledger summary", () => {
  const l = new Ledger();
...
test("binance-spot-futures-trading-bot consecutive losses", () => {
  assert.equal(maxConsecutiveLosses([-1, -2, 1, -1]), 2);
});

test("binance-spot-futures-trading-bot retry eventually succeeds", async () => {
  let n = 0;
...
test("binance-spot-futures-trading-bot price series sma", () => {
